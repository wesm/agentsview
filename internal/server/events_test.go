...
func assertCacheCleared(
	t *testing.T, sourcePath string, lastMtime int64,
) {
	t.Helper()
	if sourcePath != "" {
		t.Errorf(
			"expected sourcePath cleared, got %q", sourcePath,
		)
	}
	if lastMtime != 0 {
		t.Errorf(
			"expected lastMtime cleared, got %d", lastMtime,
		)
	}
}

func assertCachePreserved(
	t *testing.T,
	sourcePath, wantPath string,
	lastMtime, wantMtime int64,
) {
	t.Helper()
	if sourcePath != wantPath {
		t.Errorf(
			"sourcePath = %q, want %q", sourcePath, wantPath,
		)
	}
	if lastMtime != wantMtime {
		t.Errorf(
			"lastMtime = %d, want %d", lastMtime, wantMtime,
		)
	}
}
...
func TestSyncIfModified_CacheClearing(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupPath: func(t *testing.T) string {
				return "/non/existent/path/99999"
			},
			wantCleared: true,
		},
		{
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantCleared {
				...
				assertCachePreserved(
					t, sourcePath, path, lastMtime, 12345,
				)
			}
		})
	}
